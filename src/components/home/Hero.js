...
//     <section
//       id="hero"
//       className="relative flex min-h-[90vh] md:min-h-[220vh] w-full items-center overflow-hidden bg-slate"
//     >
//       <div
...
export default function HeroSection() {
  const sectionRef = useRef(null); // scope root — keeps gsap.context isolated to this section only
  const textRef = useRef(null);
  const ctaRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // Guard against a missing scope element or refs not being attached yet —
    // prevents gsap.fromTo(null, ...) from throwing.
    if (!section || !text || !cta) return;
    ...
    }, sectionRef); // scoping to sectionRef means revert() only cleans up tweens/selectors created within this section
  ...
  return (
    ...
    >
      ...
      <Image
        ...
        width={1500}
        height={1500}
        className="z-1 absolute bottom-0 right-0 w-full h-[155vh] object-cover hidden md:block"
      />
    </section>
  );
}