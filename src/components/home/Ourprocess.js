...
//                 gsap.set(linePath, {
//                   strokeDashoffset:
//                     linePath.getTotalLength() * (1 - self.progress),
//                 });
//               },
//             });
//           }
...
//                         isDown
//                           ? "order-2 mt-6 md:mt-10"
//                           : "order-1 mb-6 md:mb-10"
//                       }`}
//                     >
//                       {/* Back-left photo */}
//                       <div
...
//     let ctx; // gsap.context instance — captured synchronously in this closure, not via a promise
//     let cancelled = false; // guards against work happening after an early unmount
...
//       // If cleanup already ran while we were waiting on the imports/frames,
//       // stop here instead of creating triggers nobody will revert.
//       if (cancelled) return;
...
//       // Everything GSAP-related — the entry animation, the fade-ins, and the
//       // matchMedia (pin vs. mobile scroll) — now lives inside ONE context.
//       // ctx.revert() on cleanup will kill every ScrollTrigger created here
//       // (including the ones inside matchMedia) and undo the pin/spacer,
//       // in the right order, before React tears down the DOM.
//       ctx = gsap.context(() => {
//         if (linePath) {
//           const lineLen = linePath.getTotalLength();
//           gsap.set(linePath, {
...
//             // No manual .kill() needed — this trigger was created inside
//             // gsap.context() + matchMedia, so ctx.revert() cleans it up.
//           },
//           "(max-width: 767px)": function () {
//             // draw the line progressively as the user scrolls the whole (tall) section instead
//             if (linePath) {
...
import { STEPS } from "@/data/ourprocessData";
...
// cleanup runs AFTER — by which point GSAP's pin-spacer wrapper (from
// pin: true) has already changed this element's real parent, so React's
// removeChild call targets a stale parent and throws. On the server
// useLayoutEffect just warns, so we fall back to useEffect there.
...
export default function OurProcess() {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const initGSAP = async () => {
      ...
      syncSvgWidth();
      window.addEventListener("resize", syncSvgWidth);
      removeResizeListener = () => window.removeEventListener("resize", syncSvgWidth);
